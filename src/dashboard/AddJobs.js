...
const AddJob = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(true); 
        try {
            await axios.post('https://job-vacancy-api.vercel.app/api/jobs', jobData);
            setSuccess(true); 
            setTimeout(() => {
                navigate('/list-jobs'); 
            }, 1500); 
        } catch (error) {
            console.error('Failed to add job', error.response || error.message);
    ...
    return (
        <div className="max-w-3xl mx-auto lg:mt-24 p-6">
            ...
            {success && <div className="mb-4 p-2 bg-green-100 text-green-800 rounded">Job added successfully!</div>}
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button
                    type="submit"
                    className={`mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 ${submitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={submitting}
                >
